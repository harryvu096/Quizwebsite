...
async function j0Pick(){
  ...
  const r=await fetch("https://ce.judge0.com/languages");
...
async function runPistonAt(base, filesArr, stdin){
  const rr=await fetch(base+"/api/v2/piston/runtimes");
  ...
  const r=await fetch(base+"/api/v2/piston/execute",{method:"POST",headers:{"Content-Type":"application/json"},
...
async function runJudge0(filesArr, stdin, base){
  ...
  const r=await fetch(base+"/submissions?wait=true&base64_encoded=false",{method:"POST",
...
async function runWandbox(filesArr, stdin, base){
  ...
  for(const c of names){
    ...
    const r=await fetch(base+"/api/compile.json",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body)});
...
async function cppRun(){
  ...
  out.className="cpp-out run"; out.textContent="⏳ Compiling ("+profile.cppSettings.std.toUpperCase()+") on GCC-14 chain...";
  const S=profile.cppSettings;
  const chain=[];
  if(S.customUrl) chain.push(["Custom", ()=>runPistonAt(S.customUrl, filesArr, stdin)]);
  chain.push(["Judge0", ()=>runJudge0(filesArr, stdin)]);
  chain.push(["Wandbox", ()=>runWandbox(filesArr, stdin)]);
  CORS_PROXIES.forEach(px=>{
    chain.push(["Judge0⇄proxy", ()=>runJudge0(filesArr, stdin, px("https://ce.judge0.com"))]);
    chain.push(["Wandbox⇄proxy", ()=>runWandbox(filesArr, stdin, px("https://wandbox.org"))]);
  });
  chain.push(["Piston", ()=>runPistonAt("https://emkc.org", filesArr, stdin)]);
  let result=null; const errs=[];
  ...
  if(!result){
    ...
    out.textContent="❌ Compiler chain fail (internet/CORS/rate-limit):\n"+errs.slice(0,6).join("\n")+
      "\n\n💡 Tips: 1) dobara try karein (rate-limit)  2) Settings ⚙️ mein apna free Piston/Judge0 server URL dalein — hamesha ke liye hal.";
    return;
  }
  CPP_LAST_VIA=result.via;
  let txt="🔧 "+result.via+"  ·  "+profile.cppSettings.std.toUpperCase()+" "+profile.cppSettings.opt+(profile.cppSettings.warn?" -Wall":"")+"\n──────────────────────────────\n";
